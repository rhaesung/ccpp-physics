...
!    &                     xqrch,   mbdt,    tem,
     ...
     &                     omegaout_loc(im, km), sigmaout_loc(im, km)   
...
!           tem       = grav * zo(i,k) + cp * to(i,k)
     ...
     &                 to(i,k)
            heo(i,k)  = tem(i,k)  + real(hvap, conv_wp) * qo(i,k)
            heso(i,k) = tem(i,k)  + real(hvap, conv_wp) * qeso(i,k)
...
c
      do i=1,im
        if(cnvflg(i)) then
          ...
          ptem_1d(i) = 1.0_conv_wp - tem_1d(i)
        ...
        do i= 1, im
          if(cnvflg(i)) then
             ...
             if(tkemean(i) > tkemx) then
               ...
               tem1_1d(i) = 1.0_conv_wp - 2.0_conv_wp *  tem_1d(i) /
     &                      dtke
               clamt(i) = real(clam, conv_wp) + real(clamd, conv_wp) * 
     &                    tem1_1d(i)
             endif
        ...
        do i=1,im
          if(cnvflg(i)) then
            ...
            if(tkemean(i) > tkcrt) then
              tem_1d(i) = 1.0_conv_wp + tkemean(i)/tkcrt
...
!!  where \f$\eta\f$ is the normalized mass flux, \f$\lambda_e\f$ is the entrainment rate and \f$\lambda_d\f$ is the detrainment rate.
     ...
     &                   tem(i,k)
              eta(i,k) = eta(i,k+1)/(1.0_conv_wp+ptem(i,k)*dz)
            endif
          endif
        enddo
      enddo
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k < kmax(i)) then
              ...
              factor(i,k) = 1.0_conv_wp+tem(i,k)-tem1(i,k)
              hcko(i,k)=((1.0_conv_wp-tem1(i,k))*hcko(i,k-1)+tem(i,k)*
...
!
              ...
              ucko(i,k)=((1.0_conv_wp-tem(i,k))*ucko(i,k-1)+ptem(i,k)*
     &                real(uo(i,k), conv_wp)+ptem1(i,k)*real(uo(i,k-1), 
     &                conv_wp))/factor(i,k)
              vcko(i,k)=((1.0_conv_wp-tem(i,k))*vcko(i,k-1)+ptem(i,k)*
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k < kmax(i)) then
              ...
              tem(i,k) = cq * tem(i,k)
              factor(i,k) = 1.0_conv_wp + tem(i,k)
              ecko(i,k,n) = ((1.0_conv_wp-tem(i,k))*ecko(i,k-1,n)+
           ...
           do k = 2, km1
             do i = 1, im
               if (cnvflg(i)) then
                 if(k > kb(i) .and. k < kmax(i)) then
                   dz = zi(i,k) - zi(i,k-1)
                   tem(i,k)=0.25_conv_wp*(xlamue(i,k)+xlamue(i,k-1))*
     &                      dz
                   tem(i,k)=cq*tem(i,k)
                   factor(i,k) = 1.0_conv_wp + tem(i,k)
                   ecko(i,k,kk)=((1.0_conv_wp-tem(i,k))*ecko(i,k-1,kk)
     &              +tem(i,k)*(ctro(i,k,kk)+ctro(i,k-1,kk)))/factor(i,k)
                   ercko(i,k,kk) = ecko(i,k,kk)
                   chem_c(i,k,n) = real(fscav(n), conv_wp) *ecko(i,k,kk)
                   tem(i,k) = chem_c(i,k,n)/(1.0_conv_wp+c0t(i,k)*dz)
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k < kbcon1(i)) then
              ...
              rfact(i,k) =  1.0_conv_wp + real(fv, conv_wp) * 
...
cj
              ...
              factor(i,k) = 1.0_conv_wp + tem(i,k)
              qcko(i,k) =((1.0_conv_wp-tem(i,k))*qcko(i,k-1)+tem(i,k)*
...
c
              if(k >= kbcon(i) .and. dq > 0.0_conv_wp) then
                ...
                if(ncloud > 0 .and. k > jmin(i)) then
                  ptem(i,k) = c0t(i,k) + real(c1, conv_wp)
...
!NRL MNM: Limit overshooting not to be deeper than half the actual cloud              
              ...
              if(aa2(i) < 0.0_conv_wp .or. tem1_1d(i) >= tem_1d(i)) then
                ktcon1(i) = k
                flg(i) = .false.
              endif
            endif
...
!     diagnostic method:
     ...
     &                      (buo(i,k-1)+buo(i,k))
                     ...
                     ptem(i,k) = (1.0_conv_wp - tem(i,k)) * wu2(i,k-1)
                     ptem1(i,k) = 1.0_conv_wp + tem(i,k)
         ...
         do k = 2, km1
            do i = 1, im
               if (cnvflg(i)) then
                  if(k > kbcon1(i) .and. k < ktcon(i)) then
                     dp = 1000.0_conv_wp * del(i,k)
                     tem_1d(i) = 0.5_conv_wp * (omega_u(i,k) + 
     &                           omega_u(i,k-1))
...
!! \f$\lambda_d\f$ is the detrainment rate, \f$\beta\f$ is a constant currently set to 0.05, implying that only 5% of downdraft mass flux at LFC reaches the ground surface due to detrainment, \f$k_{LFC}\f$ is the vertical index of the LFC level, and \f$\overline{\Delta z}\f$ is the average vertical grid spacing below the LFC.
      ...
      do i = 1, im
        ...
        if(cnvflg(i)) then
          ...
          tem_1d(i)= 1.0_conv_wp/real(kd94(i), conv_wp)
          xlamd(i) = (1.0_conv_wp-beta**tem_1d(i))/dz
        endif
      enddo
c
        ...
        do i = 1, im
          if (cnvflg(i) .and. k <= kmax(i)-1) then
           if(k < jmin(i) .and. k >= kd94(i)) then
              ...
              etad(i,k) = etad(i,k+1)*(1.0_conv_wp-ptem_1d(i)*dz)
      ...
      do k = km1, 1, -1
        do i = 1, im
          if (cnvflg(i) .and. k < jmin(i)) then
              ...
              factor(i,k) = 1.0_conv_wp + tem(i,k) - tem1(i,k)
              hcdo(i,k)=((1.0_conv_wp-tem1(i,k))*hcdo(i,k+1)+tem(i,k)*
     &                  0.5_conv_wp*(heo(i,k)+heo(i,k+1)))/factor(i,k)
      ...
      do k = km1, 1, -1
        do i = 1, im
          if (cnvflg(i) .and. k < jmin(i)) then
              ...
              tem(i,k)  = real(cq, conv_wp) * tem(i,k)
              factor(i,k) = 1.0_conv_wp + tem(i,k)
              ecdo(i,k,n) = ((1.0_conv_wp-tem(i,k))*ecdo(i,k+1,n)+
     &             tem(i,k)*(ctro(i,k,n)+ctro(i,k+1,n)))/factor(i,k)
          endif
        enddo
      enddo
...
!
     ...
     &                   phkp*(qo(i,k)-real(q1(i,k+1), conv_wp))
            ...
            elseif(tem(i,k) < 0.0_conv_wp) then
              rrkp = 0.0_conv_wp
              if(abs(q_diff(i,k)) > 1.e-22_conv_wp)
     &               rrkp = q_diff(i,k-1) / q_diff(i,k)
              phkp = (rrkp+abs(rrkp)) / (1.0_conv_wp+abs(rrkp))
...
!                 xaa0(i) = xaa0(i) - dz * grav * qlk * etah
            ...
            if(k >= kbcon(i) .and. k < ktcon1(i)) then
              dz1 = zo(i,k+1) - zo(i,k)
              gamma = real(el2orc, conv_wp) * qeso(i,k) / (to(i,k)**2)
              rfact(i,k) = 1.0_conv_wp + real(fv, conv_wp) * real(cp, 
...
cj
              ...
              tem(i,k)  = real(cq, conv_wp) * tem(i,k)
              factor(i,k) = 1.0_conv_wp + tem(i,k)
              qcdo(i,k) =((1.0_conv_wp-tem(i,k))*qrcd(i,k+1)+tem(i,k)*
     &                   (real(qo(i,k), conv_wp)+real(qo(i,k+1), 
     &                   conv_wp)))/factor(i,k)
cj
...
!> - For scale-aware parameterization, the updraft fraction (sigmagfm) is first computed as a function of the lateral entrainment rate at cloud base (see Han et al.'s (2017) \cite han_et_al_2017 equation 4 and 5), following the study by Grell and Freitas (2014) \cite grell_and_freitas_2014.
     ...
     &          3.e-4_conv_wp)
          ...
          sigmagfm(i) = tem1_1d(i) / real(garea(i), conv_wp)
      ...
      do k = 1, km
        do i = 1, im
          if (cnvflg(i) .and. k <= kmax(i)) then
            ...
            uo(i,k) = real(u1(i,k), conv_wp)
            vo(i,k) = real(v1(i,k), conv_wp)
            qeso(i,k) = 0.01_conv_wp * real(fpvs(t1(i,k)), conv_wp) ! fpvs is in pa
            qeso(i,k) = real(eps, conv_wp) * qeso(i,k) / (pfld(i,k) + 
     &                  real(epsm1, conv_wp) *qeso(i,k))
            val     =             1.e-8_conv_wp
            qeso(i,k) = max(qeso(i,k), val )
...
!
     ...
     &                real(grav, conv_wp)
              ...
              if(ctr(i,k,n)<0.0_conv_wp) tsumn(i)=tsumn(i) + tem_1d(i)
              if(ctr(i,k,n)>0.0_conv_wp) tsump(i)=tsump(i) + tem_1d(i)
            endif
          enddo
        enddo
        do i = 1,im
...
!             if(islimsk(i) == 1) evef=.07
     ...
     &          exp(-.32_conv_wp*sqrt(dt2*rn(i))))
                qevap(i)=min(qevap(i), rn(i)*1000.0_conv_wp*tem_1d(i))
                delq2(i)=delqev(i)+.001_conv_wp*qevap(i)*tem1_1d(i)
              endif
              if(rn(i)>0.0_conv_wp .and. qcond(i)<0.0_conv_wp .and.
     &           delq2(i) > rntot(i)) then
                qevap(i)=1000.0_conv_wp*tem_1d(i)*(rntot(i)-delqev(i))
                flg(i) = .false.
              endif
...
!           if (k > kb(i) .and. k <= ktcon(i)) then
     ...
     &                     (tcr-real(t1(i,k), conv_wp))*tcrf))
              if (real(qtr(i,k,2), kind_phys) > -999.0_conv_wp) then
                qtr(i,k,1) = real(qtr(i,k,1), kind_phys) + 
     &                       real(tem_1d(i) * tem1_1d(i), kind_phys)  ! ice
                qtr(i,k,2) = real(qtr(i,k,2), kind_phys) + 
     &                       real(tem_1d(i) * (1.0_conv_wp -
     &                           tem1_1d(i)), kind_phys)  ! water
              else
                qtr(i,k,1) = real(qtr(i,k,1), kind_phys) + 
     &                       real(tem_1d(i), kind_phys)
              endif
            endif
          endif
        enddo
        ...
        do i = 1, im
          if(cnvflg(i) .and. rn(i) > 0.0_conv_wp) then
            if(k > kb(i) .and. k < ktop(i)) then
             ...
             tem1(i,k) = pfld(i,k) * 100.0_conv_wp / 
...
!
      do k = 2, km1
        do i = 1, im
          if(cnvflg(i) .and. rn(i) > 0.0_conv_wp) then
            if(k > 1 .and. k <= jmin(i)) then
              tem(i,k) = 0.5_conv_wp * edto(i) * (etad(i,k-1) +
     &                   etad(i,k)) * xmb(i)
              tem1(i,k) = pfld(i,k) * 100.0_conv_wp / 
     &                   (real(rd, conv_wp) * real(t1(i,k), conv_wp))
              if(progsigma) then

...
!    &,                    me
      ...
      real(kind=pbl_wp) q1(ix,km,ntrac1),
...
!
     ...
     &                     thup,    thvu,    dq
!
      real(kind=pbl_wp) rbdn(im), rbup(im), hpblx(im),
     &                     xlamue(im,km-1), xlamuem(im,km-1)
      real(kind=pbl_wp) delz(im), xlamax(im), ce0t(im)
!
      real(kind=pbl_wp) wu2(im,km), thlu(im,km),
     &                     qtx(im,km), qtu(im,km)
!
...
!  physical parameters
     ...
     &          tkcrt=2.0_pbl_wp,cmxfac=5.0_pbl_wp)
      parameter(qmin=1.e-8_pbl_wp,qlmin=1.e-12_pbl_wp)
      parameter(alp=1.5_pbl_wp,vpertmax=3.0_pbl_wp,pgcon=0.55_pbl_wp)
      parameter(b1=0.5_pbl_wp,f1=0.15_pbl_wp)
!
!************************************************************************
!!
      totflg = .true.
...
!
            thlu(i,k) = ((1.0_pbl_wp-tem)*thlu(i,k-1)+tem*
     &                  (thlx(i,k-1)+thlx(i,k)))/factor
!
            tem  = 0.5_pbl_wp * xlamueq(i,k-1) * dz
            factor = 1.0_pbl_wp + tem
...
!
      do i = 1, im
        if(cnvflg(i)) then
          dz   = zm(i,1)
          tem  = 0.5_pbl_wp*bb1*xlamue(i,1)*dz
          tem1 = bb2 * buo(i,1) * dz
          ptem1 = 1.0_pbl_wp + tem
          wu2(i,1) = tem1 / ptem1
        endif
      ...
      do k = 2, kmpbl
        do i = 1, im
          if(cnvflg(i)) then
            dz    = zm(i,k) - zm(i,k-1)
            tem  = 0.25_pbl_wp*bb1*(xlamue(i,k-1)+xlamue(i,k))*dz
            tem1 = max(wu2(i,k-1), 0.0_pbl_wp)
            tem1 = bb2 * buo(i,k) - wush(i,k) * sqrt(tem1)
            tem2 = tem1 * dz
            ptem = (1.0_pbl_wp - tem) * wu2(i,k-1)
            ptem1 = 1.0_pbl_wp + tem
...
!
            tlu = thlu(i,k) / pix(i,k)
            es = 0.01_pbl_wp * fpvs(tlu)      ! fpvs in pa
            qs = max(qmin, eps * es / (plyr(i,k)+epsm1*es))
            dq = qtu(i,k) - qs
!
            if (dq > 0.0_pbl_wp) then
...
!
      do k = 2, kmpbl
        do i = 1, im
          if (cnvflg(i) .and. k <= kpbl(i)) then
             dz   = zl(i,k) - zl(i,k-1)
             tem  = 0.5_pbl_wp * xlamuem(i,k-1) * dz
             factor = 1.0_pbl_wp + tem
             ptem = tem + pgcon
             ptem1= tem - pgcon
             ucko(i,k) = ((1.0_pbl_wp-tem)*ucko(i,k-1)+ptem*u1(i,k)
     &                    +ptem1*u1(i,k-1))/factor
             vcko(i,k) = ((1.0_pbl_wp-tem)*vcko(i,k-1)+ptem*v1(i,k)
      ...
      do k = 2, kmpbl
        do i = 1, im
          if (cnvflg(i) .and. k <= kpbl(i)) then
             ...
             factor = 1.0_pbl_wp + tem
! 
             qcko(i,k,n) = ((1.0_pbl_wp-tem)*qcko(i,k-1,n)+tem*
